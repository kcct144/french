...
4. qui 和 que 相当于英语的从句引导词 `who` 或 `that` 。不过=={.danger}两个词都没有明确的”指人“和”指物“的区别，这点和英语不同
5. 如果强调的成分是一个代词，则用重读人称代词形式。
6. 性数配合要跟随真正主语。
...
==directeur=={.danger}, **trice** n. 主任，领导，经理

==musée=={.danger} n.m. 博物馆

==travaille=={.danger}r v.i. 工作；学习

==pendant=={.danger} prép. 在…期间， 在…时候
...
==commercial, e=={.danger} adj. 商业的，贸易的
...
==entrer=={.danger} v.i. 进，进入

==si=={.danger} adv. 如此地

==magasin=={.danger} n.m. 商品，店铺

==étonné, e=={.danger} adj. 惊讶的
...
==cinquantaine=={.danger} n.f. 五十左右

==boulangerie=={.danger} n.f. 面包店

==pâtisserie=={.danger} n.f. 糕点店；糕点

==pharmacie=={.danger} n.f. 药房

==fleuriste=={.danger} n. 花店；花匠

==coiffure=={.danger} n.f. 理发；发型

==supermarché=={.danger} n.m. 超级市场
...
==épicerie=={.danger} n.f. 食品杂货店

==tabac=={.danger} n.m. 烟；烟草

==ne… que=={.danger} 仅仅，只 `only`
...
==besoin=={.danger} n.m. 需要

==avoir besoin de=={.danger} 需要……

==voisin, e=={.danger} adj. 邻近的；接近的

==quelque chose=={.danger} `something`

==avoir besoin de=={.danger}
...
==journal=={.danger} n.m. 报纸；日记

==bavarder=={.danger} v.i. 闲谈，聊天

==commerçant=={.danger} n. 商人
...
==alentours=={.danger} n.m.pl. 近郊，周围

==aux alentours de=={.danger} 在……周围

==expliquer=={.danger} v.t. 解释，讲解

==pas mal de + nom=={.danger} 不少，相当多的

如：Elle a acheté pas mal de livres en français. 她买了相当多的语法书。

==lui a expliqué Sarah=={.danger}
...
==vendre=={.danger} v.t. 卖，销售

==pratique=={.danger} adj. 方便的，实用的

==un peu de + nom=={.danger} 少量的
...
==diriger=={.danger} v.t. 领导；指导

==se diriger=={.danger} v.pr. 走向；驶向

==d'abord=={.danger} loc.adv. 首先，第一 `first`

==vers=={.danger} prép. 向，朝；将近

==rayon=={.danger} n.m. 柜台；光线

==boucherie=={.danger} n.f. 肉店

==choisir=={.danger} v.t. 挑选，选定

==kilo=={.danger} (kilogramme) n.m. 千克

==bœuf=={.danger} n.m. 牛肉；牛
...
==ensuite=={.danger} adv. 然后，后来 `then`

==produit=={.danger} n.m. 产品；产物

==rayon=={.danger} n.m. 柜台；光线

==laitier, ère=={.danger} adj. 乳品的；产奶的
...
==un gâteau au chocolat=={.danger} 巧克力蛋糕
...
==fou=={.danger} (**fol**), **folle** adj. 过多的；发狂的

==Il y a un monde fou !=={.danger}
...
==répondre=={.danger} (**à**) v.t./v.i. 回答，答复

==perdre=={.danger} v.t. 浪费；失去

> À la **sortie** du centre commercial, Claudine s'est dit : « On est plus **tranquille** à la campagne ! »
>
> 走出购物中心时，克洛蒂娜（心里）想：“（还是）乡下更加宁静啊！”

==sortie=={.danger} n.f. 出口；离开

==tranquille=={.danger} adj. 安静的；平静的
